...
class Canvas:
    def __init__(
        ...
        canvasSize: QSize = QSize(500, 500),
    ):
        self.scene = scene
        self.view = view
        ...
        self.cursurDot = self.scene.addEllipse(
            QRectF(0, 0, self.drawOption.pen.widthF(), self.drawOption.pen.widthF()),
            self.drawOption.pen,
        )
        self.cursurDot.hide()
        ...
        self.view.setCursor(
            QCursor(
                *self.toolbar.getCursors().get(
                    ...
                    self.toolbar.getCursors().get(
                        (self.drawOption.shape, False, False)
                    ),
                )
            )
        )
    ...
    def onExit(self, _: QMouseEvent):
        self.cursurDot.hide()

    def onEnter(self, _: QMouseEvent):
        ...
        if self.drawOption.shape == ERASE or self.drawOption.shape == SELECT:
            ...
            size = (
                QSizeF(self.drawOption.pen.widthF(), self.drawOption.pen.widthF())
                * self.scale()
            )
            self.cursurDot.setRect(
                QRectF(QPointF(-size.width() / 2, -size.height() / 2), size)
            )
            self.cursurDot.show()
    ...
    def onClick(self, a0: QMouseEvent):
        ...
        self.lastPos = mapped
        self.cursurDot.hide() # to avoid being right clicked on
        ...
        if opt == SELECT:
            ...
            self.currentObject.painting = True
        ...
        if opt == ERASE or a0.buttons() == Qt.MouseButton.RightButton:
            print('in right click', self.iniPos)
            item = self.view.itemAt(self.iniPos)
    ...
    def onRelease(self, a0: QMouseEvent):
        ...
        if s in (PATH, CIRCLE, RECT, SELECT):
            ...
            if opt == PATH:
                if self.lockMode == "v":
                    ...
                    self.currentLerpPos = smoothStep(
                        self.currentLerpPos, mappedCurPos, 0.19
                    )
                    self.path.lineTo(self.currentLerpPos)

                self.currentObject.setPath(self.path)

            elif opt in (CIRCLE, RECT):
    ...
    def clear(self):
        ...
        pixmap = QPixmap(self.canvas.pixmap().size())
    ...
    def redraw(self):
        ...
        for item in self.group.childItems():
            ...
            item.painting = False
            painter.drawImage(
                tempimage.rect().translated(item.pos().toPoint()), tempimage
            )
    ...
    def deleteObj(self, obj: Union[QGraphicsItem, QPointF]):
...
class CanvasTesting(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.toolbar = PaintToolbar(
            ConfigManager("d:/PythonProject/screenCap/QtExperimental/config.ini"), None
        )
        self.toolbar.show()
...
class CircleItem(QGraphicsEllipseItem):
    ...
    def paintProxy(
        self, painter: QtGui.QPainter, option: QStyleOptionGraphicsItem, widget=None
    ) -> None:
        """
...
class PathItem(QGraphicsPathItem):
    def __init__(self, filled=False, painting=True, *args, **kwargs):
